...
5. スプレッドシートへの転記機能を使う場合：[Google Cloud Console](https://console.cloud.google.com/apis/library/sheets.googleapis.com) で同じプロジェクトを選択し、「Google Sheets API」を有効にする

※ ログインはポップアップウィンドウ方式です。ブラウザでこのサイトのポップアップがブロックされないようにしてください（ブロックされた場合はアドレスバー付近に表示が出るので、許可を選んでください）。
...
## 出勤・退勤ボタン
「入力」タブの一番上に、作業記録とは別に「🕐 出勤する／退勤する」ボタンがあります。
- 押すと出勤時刻・位置情報が記録され、もう一度押すと退勤時刻が記録されます
- 出勤中は下に「☕ 休憩する」ボタンが表示されます。押すと休憩開始、もう一度押すと休憩終了が記録されます（複数回の休憩も記録できます）
- 退勤時に休憩時間が自動で差し引かれ、「勤務時間」と「休憩時間」が分けて表示されます
- 退勤し忘れたまま休憩中だった場合も、退勤ボタンを押せば自動的に休憩も終了扱いになります
- ボタンの下に、その日出勤している全員の状況（勤務中／休憩中／退勤済み）がリアルタイムで表示されます
- 作業内容ごとの記録（GPSで作業開始・終了）とは別の記録なので、両方使うと「1日の総労働時間」と「作業内訳」の両方を把握できます

## Googleスプレッドシートへの転記
「日報」タブの「📊 スプレッドシートに転記」ボタンで、表示中の日付のデータをGoogleスプレッドシートに追記できます（毎晩ご自身で押していただく手動方式です）。

**準備**
1. 転記先にしたいGoogleスプレッドシートを新規作成（またはお持ちのものを用意）
2. ブラウザのURL欄から、`/d/` と `/edit` の間にある文字列（スプレッドシートID）をコピー
   - 例：`https://docs.google.com/spreadsheets/d/【この部分】/edit`
3. アプリの ⚙設定 →「Googleスプレッドシートに転記先ID」に貼り付け（URLをそのまま貼ってもOKです、自動で抽出します）
4. 「作業記録：シート名・開始セル」は、実際のシートのタブ名に合わせて設定してください（日本語環境のデフォルトは`シート1`であることが多いです。例：`シート1!A1`）
5. 出退勤も転記したい場合は、スプレッドシート内に新しいタブ（シート）をもう1つ追加し、「出退勤：シート名・開始セル」にそのタブ名を設定してください（例：`出退勤!A1`）
6. 「保存」を押す

**列の並び（作業記録）**：日付／開始／終了／作業時間／作業者／圃場／作物／作業内容／メモ／GPS開始リンク／GPS終了リンク
**列の並び（出退勤）**：日付／作業者／出勤／退勤／勤務時間（休憩差引後）／休憩時間／GPS出勤リンク／GPS退勤リンク
（どちらも1行目に見出しは自動で入りません。最初に1回だけご自身で見出し行を入れておくと分かりやすいです）

**初回の注意**：ボタンを押すと、Googleへのログイン画面（スプレッドシートへのアクセス許可）が再度表示されることがあります。これはFirebaseへのログインとは別に、Sheetsへの書き込み許可を確認するためのものです。許可すると転記されます。

**エラーが出た場合**
- 「権限がありません」：スプレッドシートIDが正しいか、ログイン中のGoogleアカウントがそのシートを編集できるか確認してください
- 「スプレッドシートが見つかりません」：IDの貼り付けミスがないか確認してください
