...
4. 「シート名・開始セル」は初期値`Sheet1!A1`のままで大丈夫です（シートのタブ名を変えている場合はそこに合わせて書き換えてください）
5. 「保存」を押す

**列の並び**：日付／開始／終了／作業時間／作業者／圃場／作物／作業内容／メモ／GPS開始リンク／GPS終了リンク
（1行目に見出しは自動で入りません。最初に1回だけご自身で見出し行を入れておくと分かりやすいです）
